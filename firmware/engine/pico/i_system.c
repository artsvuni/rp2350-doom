...
#if PICO_ON_DEVICE
    bootlog_print("z1: AutoAllocMemory entered");
#endif
    while (zonemem == NULL)
    {
        ...
        // Zone memory MUST come from the __end__..SHORTPTR_BASE+0x40000
        // window regardless of USE_ZONE_FOR_MALLOC (which we keep off -
        // enabling it makes i_system.c's own __wrap_malloc/calloc/free
        // collide with pico_malloc's, see DECISIONS.md): the engine's
        // "short pointer" compression (doomtype.h's ptr_to_shortptr())
        // asserts every zone pointer falls in that exact range, and hits
        // an unconditional `bkpt` (not a graceful assert) otherwise -
        // silent hang with no debugger attached. A plain malloc()'d
        // pointer from the general newlib heap has no such guarantee.
        // This used to be gated on USE_ZONE_FOR_MALLOC (upstream always
        // sets both together); we need it unconditionally here.
#if PICO_ON_DEVICE
...
#if PICO_ON_DEVICE
    bootlog_print("z4: AutoAllocMemory returned");
#endif

    printf("zone memory: %p, %x allocated for zone\n",
           zonemem, *size);
#if PICO_ON_DEVICE
    bootlog_print("z5: I_ZoneBase printf done");
#endif
